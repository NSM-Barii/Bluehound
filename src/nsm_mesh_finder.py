...
class BLE_Sniffer(): 
    ...
    def main(cls, war_drive=False, print=False, server_ip=False):
        ...
        Extensions.server_ip = server_ip
        ...
        try:
            ...
            if False:
                if war_drive or print: 
                    threading.Thread(target=asyncio.run(BLE_Sniffer._ble_printer), args=(war_drive, print, server_ip), daemon=True).start()
                    from nsm_server import Web_Server; threading.Thread(target=Web_Server.start, args=(console, ), daemon=True).start(); time.sleep(1)
                    while True: time.sleep(1)
                
            elif war_drive or print: 
                from nsm_server import Web_Server; threading.Thread(target=Web_Server.start, args=(console, ), daemon=True).start(); time.sleep(1)
